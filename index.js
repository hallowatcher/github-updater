...
Updater.prototype.checkForUpdates = function () {
    request.get(`https://api.github.com/repos/${this.config.repository}/releases`, (error, response, body) => {
        if (error) return this.emit('error', new Error('Could not connect to repo!'));
        
        try {
            var data = JSON.parse(body);

            if (!data) throw new Error('No data was received!');
            if (data.length === 0) throw new Error('No releases have been made!');
            if (!semver.valid(this.config.version)) throw new Error('The version specified is invalid!');
            if (!semver.valid(data[0].tag_name)) throw new Error('The latest release in github has an invalid tag version!');

            let localVer = semver.clean(this.config.version);
            let latestVer = semver.clean(data[0].tag_name);

            if (semver.gt(latestVer, localVer)) {
                if (!data[0].assets) throw new Error('The latest version has no assets!');
